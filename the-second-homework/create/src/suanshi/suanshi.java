...
public class suanshi {
        ...
        private int max,
                Brackets,
                Decimal;
        private double[] Num_Con=new double[10],
                shu_new=new double[10];
        ...
        public suanshi(int Max, int tyle)
        {
                max=Max;
        ...
        public void Creats()
        {
                Num_Con[0]=getnum();//随机获取数字放进数组
                for(int i=1;i<=(int)(Math.random()*3)+1;i++)//生成一个1-4的随机数
                {
                        Num_Con[i]=getnum();//获取随机数字
                        getchar();//获取一个符号
                        Symbol_Con=Symbol_Con+symbol_sel;//将符号搞成一串
                }
                Creat_File();//创建文件
                Display();
                QuiZ=QuiZ+"=";
        ...
        private double getnum()//获取一个数字1~最大的
        {
                double a=(Math.random()*max)+1;
                a=((int)(a*100))/100.0;//变成只有一位小数
                return (a);
        }
        ...
        public void getchar()
        {
                ...
                symbol_sel=symbol.substring(Mark, Mark+1);//获取
        }
}
